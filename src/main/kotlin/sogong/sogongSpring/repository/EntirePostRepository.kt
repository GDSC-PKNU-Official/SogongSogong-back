...
interface EntirePostRepository : JpaRepository<EntirePostEntity, Long> {
    ...
    @Query(value = "select * from ENTIRE_POST where countComment >= :countComment " +
            "order by postId desc limit 20", nativeQuery = true)
    ...
    @Query(value = "select * from ENTIRE_POST where postId < :postId order by postId desc limit 20",
        nativeQuery=true)
    fun findEntirePostByLastId(@Param("postId") lastPost:Long) : List<EntirePostEntity>

    @Query(value = "select * from ENTIRE_POST order by postId desc limit 20", nativeQuery=true)
    fun findEntirePost():List<EntirePostEntity>

    @Query(value = "select * from ENTIRE_POST where postId in :ids order by postId desc", nativeQuery=true)
    fun findPostByIds(@Param(value="ids") postIds:List<Long>) : List<EntirePostEntity>
}